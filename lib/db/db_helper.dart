...
import 'package:user_ecomm/models/cart_model.dart';
import 'package:user_ecomm/models/comment_model.dart';
...
class DbHelper {
  ...
  static Stream<QuerySnapshot<Map<String, dynamic>>> getOrderByUserId(String uid) =>
  ...
  static Future<QuerySnapshot<Map<String, dynamic>>> getCommentByProduct(String proId) => _db
          .collection(collectionProduct)
          .doc(proId)
  ...
  static Future<void> addToCart(String uid, CartModel cartModel) {
    return _db.collection(collectionUser).doc(uid).collection(collectionCart)
  ...
  static removeFromCart(String uid, String pid) {
    return  _db
  ...
  static Future<void> saveOrder(OrderModel orderModel) async{
    ...
    for(final cartModel in orderModel.productDetails){
      final productSnapshot = await _db.collection(collectionProduct)
          .doc(cartModel.productId)
          .get();
      final productModel = ProductModel.fromMap(productSnapshot.data()!);
      ...
      final newProductModelInCategory = categoryModel.productCount - cartModel.quantity;
      ...
      wb.update(catDoc, {categoryFieldProductCount : newProductModelInCategory});
    }
  ...
  static Future<void> addNoticfication(NotificationModel notification) {
